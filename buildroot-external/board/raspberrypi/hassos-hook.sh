...
function hassos_pre_image() {
    ...
    if [ -n "$DISTRO_RECOVERY_IMAGE" ]; then
        more="systemd.unit=recovery.target"
    fi
    echo "dwc_otg.lpm_enable=0 console=tty1 $more usb-storage.quirks=174c:55aa:u,2109:0715:u,152d:1576:u,152d:0578:u,152d:1561:u,174c:0829:u,174c:1153:u" > "${BOOT_DATA}/cmdline.txt"
